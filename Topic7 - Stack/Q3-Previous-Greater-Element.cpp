...
vector<int> previousGreaterElement(vector<int> nums)
{
  ...
  // stack me prev greater element store krne ke liye
  stack<int> st;
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    // tab tak stack ka top check krte rho ke nums[i] se small or equal jo elements hai unko pop krte rho
    ...
    // iss ka matlab hai ke stack ke top me greater element hai
    // so result me stack ka greater element push kro
    else
    {
      res[i] = st.top();
    }
