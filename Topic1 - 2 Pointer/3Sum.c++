...
int threeSumClosest(vector<int> &nums, int target)
{
  ...
  for (int i = 0; i < n - 2; i++)
  {
    ...
    while (left < right)
    {
      ...
      // check kro ki current difference best difference se chota hai ya nhi
      if (currentDiff < bestDiff)
      {
        // agar chota hai to update kro best difference ko
        bestDiff = currentDiff;

        // and then update kro best sum ko
        bestSum = currentSum;
      }
      ...
      // agar sum target se bada hai to right pointer ko peeche le jao
      else
      {
        right--;
      }
